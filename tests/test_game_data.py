...
class TestGameParams(BaseClass):
    ...
    def test_engines(self, get_data):
        ...
        assert (set(engine_name), engine_power, engine_type) == (set(fake_engine_name), fake_engine_power, fake_engine_type),\
    ...
    def test_hulls(self, get_data):
        ...
        assert (set(hull_name), hull_armor, hull_type, hull_capacity) ==\
               (set(fake_hull_name), fake_hull_armor, fake_hull_type, fake_hull_capacity), \
    ...
    def test_weapons(self, get_data):
        ...
        fake_diameter, fake_power, fake_count = int(fake_data['diameter']), int(fake_data['power_volley']),\
                                                                                                int(fake_data['count'])
        assert (set(name), reload_speed, rotation, diameter, power, count) == \
